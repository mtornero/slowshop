...
abstract class Order implements ActiveRecordInterface
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\App\\Propel\\Map\\OrderTableMap';
    ...
    /**
     * Initializes internal state of App\Propel\Base\Order object.
     */
    ...
    public function setDeliveryId($v)
    {
        ...
        if ($this->delivery_id !== $v) {
            $this->delivery_id = $v;
            $this->modifiedColumns[OrderTableMap::COL_DELIVERY_ID] = true;
        }
     ...
     * @return $this|\App\Propel\Order The current object (for fluent API support)
     */
    public function setOrderComment($v)
    {
        if ($v !== null) {
            $v = (string) $v;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : OrderTableMap::translateFieldName('DeliveryId', TableMap::TYPE_PHPNAME, $indexType)];
            $this->delivery_id = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 3 + $startcol : OrderTableMap::translateFieldName('OrderComment', TableMap::TYPE_PHPNAME, $indexType)];
            $this->order_comment = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 4 + $startcol : OrderTableMap::translateFieldName('CreatedAt', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            throw new PropelException(sprintf('Error populating %s object', '\\App\\Propel\\Order'), 0, $e);
        }
    }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(OrderTableMap::COL_DELIVERY_ID)) {
    ...
    public function fromArray($arr, $keyType = TableMap::TYPE_PHPNAME)
    {
        ...
        if (array_key_exists($keys[2], $arr)) {
            $this->setDeliveryId($arr[$keys[2]]);
        }
        if (array_key_exists($keys[3], $arr)) {
            $this->setOrderComment($arr[$keys[3]]);
        }
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\App\Propel\Order The current object, for fluid interface
     */
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(OrderTableMap::COL_DELIVERY_ID)) {
            $criteria->add(OrderTableMap::COL_DELIVERY_ID, $this->delivery_id);
        }
        if ($this->isColumnModified(OrderTableMap::COL_ORDER_COMMENT)) {
     ...
     * objects.
     *
     * @param      object $copyObj An object of \App\Propel\Order (or compatible) type.
    ...
    public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
    {
        ...
        $copyObj->setOrderComment($this->getOrderComment());
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \App\Propel\Order Clone of current object.
     * @throws PropelException
     */
